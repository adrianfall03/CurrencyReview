...
const I18N = {
  en: {
    ...
    app_subtitle: 'Accounting-friendly FX rates (Mizuho / ECB / Mitsubishi MURC)',
  ...
  ja: {
    app_title: '為替換算',
    app_subtitle: '会計向け為替レート（みずほ / ECB / 三菱MURC）',
    label_source: 'データソース',
    label_provider: '提供元',
...
export default function FXConverter() {
  ...
  useEffect(() => {
    const brand = source === 'mizuho' ? 'mizuho' : source === 'murc' ? 'murc' : 'ecb'
  ...
  const loadCurrencies = useCallback(
    async (src: Source, refresh = false) => {
      ...
      try {
        ...
        setLoaded(true)
        setLoadedLabel(
          src === 'mizuho' ? t('source_mizuho') :
          src === 'ecb' ? t('source_ecb') :
          t('source_murc')
        )
  ...
  const basisLabel =
    source === 'murc' ? basis.toUpperCase() :
    source === 'mizuho' ? 'TTM' : 'MID'
